...
use opportunity_runtime::{self, opaque::Block, RuntimeApi};
...
> {
	...
	let (grandpa_block_import, grandpa_link) =
		sc_finality_grandpa::block_import(client.clone(), &(client.clone() as Arc<_>), select_chain.clone(), telemetry.as_ref().map(|(_, telemetry)| telemetry.handle()) )?;
...
> {
	...
	let _rpc_handlers = sc_service::spawn_tasks(sc_service::SpawnTasksParams {
		...
		system_rpc_tx,
		telemetry: telemetry.as_mut().map(|(_, telemetry)| telemetry)
	})?;
	...
	let grandpa_config = sc_finality_grandpa::Config {
		...
		telemetry: telemetry.as_mut().map(|(_, telemetry)| telemetry.handle())
	};
...
pub fn new_light(config: Configuration) -> Result<TaskManager, ServiceError> {
	...
	let import_queue = sc_consensus_babe::import_queue(
		...
		telemetry.as_mut().map(|(_, telemetry)| telemetry.handle())
	)?;
