...
fn load_spec(id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
	Ok(match id {
		...
		"standard-barocco" => Box::new(StandardChainSpec::from_json_bytes(
			&include_bytes!("../spec/standard_barocco_raw.json")[..],
		)?),
		path => Box::new(StandardChainSpec::from_json_file(std::path::PathBuf::from(path))?),
	})
}

impl SubstrateCli for Cli {
	fn impl_name() -> String {
		"Standard Collator".into()
	}
	...
	fn support_url() -> String {
		"https://github.com/digitalnativeinc/standard-substrate/issues/new".into()
	}
			...
			match cmd {
				BenchmarkCmd::Pallet(cmd) =>
					if cfg!(feature = "runtime-benchmarks") {
						runner.sync_run(|config| cmd.run::<Block, StandardRuntimeExecutor>(config))
					} else {
				...
				BenchmarkCmd::Block(cmd) => runner.sync_run(|config| {
					let partials = new_partial::<RuntimeApi, StandardRuntimeExecutor, _>(
						&config,
						crate::service::parachain_build_import_queue,
					)?;
