...
fn create_asset() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(
            AssetRegistryModule::get_or_create_asset(b"DOT".to_vec()),
            dot_asset_id
        );
        assert_eq!(AssetRegistryModule::next_asset_id(), current_asset_id);
        assert_eq!(
            AssetRegistryModule::asset_ids(b"DOT".to_vec()).unwrap(),
            1u32
        );
        assert_eq!(
            AssetRegistryModule::asset_ids(b"AAA".to_vec()).is_none(),
            true
        );
    });
}
